...
announce_init_bins rnsd rnx rnid
...
run_direction() {
  ...
  if ! wait_for_new_log_match "$START_TIMEOUT_SECS" "$receiver_log" "$receiver_from_line" "$announce_pattern"; then
    echo "$(announce_prefix) $label: receiver did not log announce; see $receiver_log"
    stop_proc "$listener_pid"; stop_proc "$sender_side_pid"; stop_proc "$receiver_pid"
    return 1
  fi
...
if [[ "$overall" -ne 0 ]]; then
  echo "$(announce_prefix) rnx $ANNOUNCE_INTERFACE_LABEL announce checks FAILED"
  exit 1
fi

echo "$(announce_prefix) rnx $ANNOUNCE_INTERFACE_LABEL announce checks passed"
