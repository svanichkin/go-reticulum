...
announce_wait_sender_ready() {
  ...
  echo "$(announce_prefix) $label: note: these configs intentionally tolerate one bootstrap interface failing; only zero observed bootstrap traffic is a bootstrap readiness failure"
  return 1
}
