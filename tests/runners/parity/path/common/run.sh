...
echo "[parity-path-$ANNOUNCE_INTERFACE_NAME] dir=$ANNOUNCE_INTERFACE_DIR"
echo "[parity-path-$ANNOUNCE_INTERFACE_NAME] docker=$USE_DOCKER"
echo "[parity-path-$ANNOUNCE_INTERFACE_NAME] parallel=$PARALLEL"

build_runner_image() {
  if [[ "$USE_DOCKER" != "1" ]]; then
    return 0
  fi
  echo "[parity-path-$ANNOUNCE_INTERFACE_NAME] docker build $RUNNER_IMAGE"
...
if [[ "${#scripts[@]}" -eq 0 ]]; then
  echo "[parity-path-$ANNOUNCE_INTERFACE_NAME] no $ANNOUNCE_INTERFACE_NAME path parity scripts yet"
  exit 0
fi
...
if [[ "$PARALLEL" == "1" ]]; then
  ...
  for script in "${scripts[@]}"; do
    ...
    echo "[parity-path-$ANNOUNCE_INTERFACE_NAME] start $name -> $log"
  ...
  for i in "${!pids[@]}"; do
    ...
    echo "[parity-path-$ANNOUNCE_INTERFACE_NAME] ${names[$i]} exit=$code log=${logs[$i]}"
  done
...
for script in "${scripts[@]}"; do
  name="$(basename "$script")"
  echo "[parity-path-$ANNOUNCE_INTERFACE_NAME] $name"
  run_script "$script"
done
