...
OUT_DIR="$ROOT/tests/artifacts/logs/$TS/compare_request_response_two_nodes_udp"
mkdir -p "$OUT_DIR"
...
run_pair() {
  ...
  bash -lc "env HOME=\"$home_b\" USERPROFILE=\"$home_b\" $helper_cmd_b $cfg_flag_b \"$node_b_dir\" --listen --wait-seconds \"$WAIT_SECS\"" >"$listener_out" 2>&1 &
  local listener_pid=$!
  ...
  local code
  code="$(run_capture "$client_out" bash -lc "env HOME=\"$home_a\" USERPROFILE=\"$home_a\" $helper_cmd_a $cfg_flag_a \"$node_a_dir\" --destination \"$listen_hash\" --wait-seconds \"$WAIT_SECS\"")"
  if [[ "$code" != "0" ]]; then
  ...
  assert_log_contains "$client_out" "EVENT echo_response" "$label" || { stop_proc "$listener_pid"; return 1; }
  assert_log_contains "$client_out" "EVENT sleep_response" "$label" || { stop_proc "$listener_pid"; return 1; }
  assert_log_contains "$client_out" "EVENT denied_" "$label" || { stop_proc "$listener_pid"; return 1; }
  assert_log_contains "$client_out" "EVENT timeout_" "$label" || { stop_proc "$listener_pid"; return 1; }
  assert_log_contains "$client_out" "EVENT malformed_" "$label" || { stop_proc "$listener_pid"; return 1; }
...
if ! run_pair "go_node_a_python_node_b" \
  "$GO_BIN_DIR/go_request_helper" "-config" \
  "$PYTHON $ROOT/tests/support/tools/py_request_helper.py" "--config"; then
  overall=1
fi
if ! run_pair "python_node_a_go_node_b" \
  "$PYTHON $ROOT/tests/support/tools/py_request_helper.py" "--config" \
  "$GO_BIN_DIR/go_request_helper" "-config"; then
  overall=1
fi
if [[ "$overall" != "0" ]]; then
  echo "[cmp] FAIL"
