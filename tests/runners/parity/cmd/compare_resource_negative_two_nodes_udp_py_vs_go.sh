...
OUT_DIR="$ROOT/tests/artifacts/logs/$TS/compare_resource_negative_two_nodes_udp"
mkdir -p "$OUT_DIR"
...
run_mode() {
  ...
  local helper_cmd_a="$3" cfg_flag_a="$4"
  local helper_cmd_b="$5" cfg_flag_b="$6"
  ...
  bash -lc "env HOME=\"$home_b\" USERPROFILE=\"$home_b\" $helper_cmd_b $cfg_flag_b \"$node_b_dir\" --listen --mode \"$mode\" --small-payload \"$SMALL_PAYLOAD\" --wait-seconds \"$WAIT_SECS\"" >"$listener_out" 2>&1 &
  local listener_pid=$!
  ...
  local code
  code="$(run_capture "$sender_out" bash -lc "env HOME=\"$home_a\" USERPROFILE=\"$home_a\" $helper_cmd_a $cfg_flag_a \"$node_a_dir\" --mode \"$mode\" --destination \"$listen_hash\" --small-payload \"$SMALL_PAYLOAD\" --wait-seconds \"$WAIT_SECS\"")"
  if [[ "$code" != "0" ]]; then
  ...
  if [[ "$mode" == "reject" ]]; then
    rg --fixed-strings "EVENT rejected" "$sender_out" >/dev/null 2>&1 || { echo "[cmp] $label/$mode: reject missing"; stop_proc "$listener_pid"; return 1; }
  else
    rg --fixed-strings "EVENT canceled" "$sender_out" >/dev/null 2>&1 || { echo "[cmp] $label/$mode: cancel missing"; stop_proc "$listener_pid"; return 1; }
  fi
...
run_pair() {
  ...
  echo
  echo "[cmp] $label: negative resource flows"
  run_mode "$label" reject "$helper_cmd_a" "$cfg_flag_a" "$helper_cmd_b" "$cfg_flag_b" || return 1
  run_mode "$label" cancel "$helper_cmd_a" "$cfg_flag_a" "$helper_cmd_b" "$cfg_flag_b" || return 1
  echo "[cmp] $label OK"
}
