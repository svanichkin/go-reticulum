...
OUT_DIR="$ROOT/tests/artifacts/logs/$TS/compare_channel_two_nodes_udp"
mkdir -p "$OUT_DIR"
...
go build -o "$GO_BIN_DIR/go_channel_helper" ./tests/support/tools/go_channel_helper
...
run_pair() {
  ...
  local code
  code="$(run_capture "$sender_out" bash -lc "env HOME=\"$home_a\" USERPROFILE=\"$home_a\" $helper_cmd_a $cfg_flag_a \"$node_a_dir\" --destination \"$listen_hash\" --wait-seconds \"$WAIT_SECS\"")"
  if [[ "$code" != "0" ]]; then
  ...
  wait_for_file_contains "$START_TIMEOUT_SECS" "$listener_out" "EVENT received 1 alpha" || { echo "[cmp] $label: msg1 missing"; stop_proc "$listener_pid"; return 1; }
  wait_for_file_contains "$START_TIMEOUT_SECS" "$listener_out" "EVENT received 2 beta" || { echo "[cmp] $label: msg2 missing"; stop_proc "$listener_pid"; return 1; }
  wait_for_file_contains "$START_TIMEOUT_SECS" "$listener_out" "EVENT received 3 gamma" || { echo "[cmp] $label: msg3 missing"; stop_proc "$listener_pid"; return 1; }
...
if ! run_pair "go_node_a_python_node_b" \
  "$GO_BIN_DIR/go_channel_helper" "-config" \
  "$PYTHON $ROOT/tests/support/tools/py_channel_helper.py" "--config"; then
  overall=1
fi
if ! run_pair "python_node_a_go_node_b" \
  "$PYTHON $ROOT/tests/support/tools/py_channel_helper.py" "--config" \
  "$GO_BIN_DIR/go_channel_helper" "-config"; then
  overall=1
fi
if [[ "$overall" != "0" ]]; then
  echo "[cmp] FAIL"
