...
OUT_DIR="$ROOT/tests/artifacts/logs/$TS/compare_request_response_two_nodes_tcp"
mkdir -p "$OUT_DIR"
...
go build -o "$GO_BIN_DIR/go_request_helper" ./tests/support/tools/go_request_helper
...
wait_for_any_bootstrap_up() {
  ...
  while true; do
    _="$(eval "$cmd_a" || true)"
    _="$(eval "$cmd_b" || true)"
    if rg -F "Bootstrap TCP 21223388164" "$out_a" >/dev/null 2>&1 && rg -F "Status    : Up" "$out_a" >/dev/null 2>&1; then return 0; fi
    if rg -F "Bootstrap TCP 21223388164" "$out_b" >/dev/null 2>&1 && rg -F "Status    : Up" "$out_b" >/dev/null 2>&1; then return 0; fi
    if rg -F "Bootstrap TCP Dublin" "$out_a" >/dev/null 2>&1 && rg -F "Status    : Up" "$out_a" >/dev/null 2>&1; then return 0; fi
    if rg -F "Bootstrap TCP Dublin" "$out_b" >/dev/null 2>&1 && rg -F "Status    : Up" "$out_b" >/dev/null 2>&1; then return 0; fi
    local now
    now="$(date +%s)"
    if [[ $((now - start)) -ge "$timeout" ]]; then
      return 1
...
run_pair() {
  ...
  env HOME="$home_a" USERPROFILE="$home_a" $rnsd_cmd_a $cfg_flag_a "$node_a_dir" -q >"$log_a" 2>&1 &
  local pid_a=$!
  env HOME="$home_b" USERPROFILE="$home_b" $rnsd_cmd_b $cfg_flag_b "$node_b_dir" -q >"$log_b" 2>&1 &
  local pid_b=$!
  sleep 0.5
  maybe_skip_env "$log_a"
  maybe_skip_env "$log_b"
  wait_for_file_contains "$START_TIMEOUT_SECS" "$log_a" "Started rnsd version" || { stop_proc "$pid_a"; stop_proc "$pid_b"; return 1; }
  wait_for_file_contains "$START_TIMEOUT_SECS" "$log_b" "Started rnsd version" || { stop_proc "$pid_a"; stop_proc "$pid_b"; return 1; }

  local status_a="$OUT_DIR/${label}.bootstrap.status.node_a.out"
  local status_b="$OUT_DIR/${label}.bootstrap.status.node_b.out"
  wait_for_tcp_status "$START_TIMEOUT_SECS" "$status_a" 'TCP Client/' env HOME="$home_a" USERPROFILE="$home_a" $rnstatus_cmd_a $cfg_flag_a "$node_a_dir" -a || { stop_proc "$pid_a"; stop_proc "$pid_b"; return 1; }
  wait_for_tcp_status "$START_TIMEOUT_SECS" "$status_b" 'TCP Server/' env HOME="$home_b" USERPROFILE="$home_b" $rnstatus_cmd_b $cfg_flag_b "$node_b_dir" -a || { stop_proc "$pid_a"; stop_proc "$pid_b"; return 1; }
  wait_for_any_bootstrap_up "$START_TIMEOUT_SECS" "$status_a" "$status_b" \
    "env HOME='$home_a' USERPROFILE='$home_a' $rnstatus_cmd_a $cfg_flag_a '$node_a_dir' -a >'$status_a' 2>&1" \
    "env HOME='$home_b' USERPROFILE='$home_b' $rnstatus_cmd_b $cfg_flag_b '$node_b_dir' -a >'$status_b' 2>&1" || { stop_proc "$pid_a"; stop_proc "$pid_b"; return 1; }
  echo "[cmp] $label: bootstrap_ok=yes"
  stop_proc "$pid_a"
  stop_proc "$pid_b"

  echo "[cmp] $label: local TCP mixed request/response phase"
  ...
  env HOME="$home_b" USERPROFILE="$home_b" \
    $helper_cmd_b $cfg_flag_b "$node_b_dir" --identity "$listener_identity" --listen --wait-seconds "$WAIT_SECS" >"$listener_log" 2>&1 &
  local listener_pid=$!
  wait_for_file_contains "$START_TIMEOUT_SECS" "$listener_log" "LISTEN_HASH " || { stop_proc "$listener_pid"; return 1; }
  local listen_hash
  listen_hash="$(extract_listen_hash "$listener_log")"
  [[ -n "$listen_hash" ]] || { stop_proc "$listener_pid"; return 1; }
  sleep 2

  local client_code
  client_code="$(run_capture "$client_log" env HOME="$home_a" USERPROFILE="$home_a" \
    $helper_cmd_a $cfg_flag_a "$node_a_dir" --identity "$client_identity" --destination "$listen_hash" --wait-seconds "$WAIT_SECS")"
