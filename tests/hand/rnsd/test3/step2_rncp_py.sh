...
WORK_DIR="$ARTIFACTS_DIR/run/cross-py"
...
rm -rf "$WORK_DIR"
mkdir -p "$WORK_DIR/py_to_go/src" "$WORK_DIR/py_to_go/recv" "$WORK_DIR/go_to_py/src" "$WORK_DIR/go_to_py/recv"

echo "[2/7] Go listener on Go server node"
...
echo "[3/7] Python client sends file to Go server"
SRC="$WORK_DIR/py_to_go/src/python-to-go.txt"
printf "python-to-go-%s\n" "$(date +%s%N)" >"$SRC"
if ! run_with_rncp_retries "$WORK_DIR/py_to_go/send.py.out" \
  env PYTHONPATH="$ROOT/python" PYTHONUNBUFFERED=1 \
  "$PYTHON" "$RNCP_PY" --config "$PY_CLIENT_DIR" -S -w 30 "$SRC" "$DEST"; then
  echo "Python client could not send $SRC to Go destination $DEST"
  cat "$LISTENER_LOG"
  cat "$WORK_DIR/py_to_go/send.py.out"
  exit 1
fi
RECV="$WORK_DIR/py_to_go/recv/$(basename "$SRC")"
if ! wait_for_file "$RECV"; then
  echo "Go server did not receive $RECV"
  cat "$LISTENER_LOG"
  cat "$WORK_DIR/py_to_go/send.py.out"
  exit 1
fi
if [[ "$(sha256_file "$SRC")" != "$(sha256_file "$RECV")" ]]; then
  echo "Python->Go payload hash mismatch"
  exit 1
fi
stop_listener

echo "[4/7] Python listener on Python server node"
...
echo "[5/7] Go client sends received data back to Python server"
SRC_BACK="$WORK_DIR/go_to_py/src/go-back-to-python.txt"
cp "$RECV" "$SRC_BACK"
if ! run_with_rncp_retries "$WORK_DIR/go_to_py/send.go.out" \
  "$RNCP_BIN" -config "$GO_CLIENT_DIR" -S -w 30 "$SRC_BACK" "$DEST"; then
  echo "Go client could not send $SRC_BACK to Python destination $DEST"
  cat "$LISTENER_LOG"
  cat "$WORK_DIR/go_to_py/send.go.out"
  exit 1
fi
RECV_BACK="$WORK_DIR/go_to_py/recv/$(basename "$SRC_BACK")"
if ! wait_for_file "$RECV_BACK"; then
  echo "Python server did not receive $RECV_BACK"
  cat "$LISTENER_LOG"
  cat "$WORK_DIR/go_to_py/send.go.out"
  exit 1
fi
if [[ "$(sha256_file "$SRC")" != "$(sha256_file "$RECV_BACK")" ]]; then
  echo "Go->Python return payload hash mismatch"
  exit 1
fi

echo "[6/7] Artifacts"
echo "original: $SRC"
echo "python->go received: $RECV"
echo "go->python returned: $RECV_BACK"
