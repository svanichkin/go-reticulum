...
SHARED_PORT=37436
CONTROL_PORT=37437
INSTANCE_NAME="test5-go"
RNSD_FLAGS=(-config "$CFG" -vvvvvvv)
...
echo "[3/4] Starting rnsd"
echo "Command: $RNSD_BIN ${RNSD_FLAGS[*]}"
"$RNSD_BIN" "${RNSD_FLAGS[@]}" >"$LOGFILE" 2>&1 &
RNSD_PID=$!
echo "$RNSD_PID" > "$PIDFILE"

echo "[4/4] Waiting for startup logs"
started=0
ready=0
for _ in {1..200}; do
  if [[ -f "$LOGFILE" ]]; then
...
if [[ "$started" -ne 1 || "$ready" -ne 1 ]]; then
  echo "Timed out waiting for rnsd startup"
  [[ -f "$LOGFILE" ]] && cat "$LOGFILE"
  exit 1
fi
