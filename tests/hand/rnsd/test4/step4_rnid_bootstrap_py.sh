...
OBSERVER_TIMEOUT="${OBSERVER_TIMEOUT:-20}"
OBSERVER_SETTLE="${OBSERVER_SETTLE:-5}"
...
require_runtime() {
  ...
  if [[ ! -f "$dir/config" ]]; then
    ...
    echo "Run ./tests/hand/rnsd/test4/step3_rnsd_bootstrap_py.sh first"
    exit 1
  fi
}
...
relay_b_hits="$(grep -F -c "Destination <${DEST_HASH}> is now" "$RELAY_B_LOG" || true)"
relay_c_hits="$(grep -F -c "Destination <${DEST_HASH}> is now" "$RELAY_C_LOG" || true)"
origin_hits="$(grep -F -c "Rebroadcasting announce for <${DEST_HASH}>" "$ORIGIN_LOG" || true)"
